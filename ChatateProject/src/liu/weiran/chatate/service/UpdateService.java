...
public class UpdateService {
									...
									.setPositiveButton(
											...
											new DialogInterface.OnClickListener() {
												...
												public void onClick(
														...
														int which) {
													openUrlInBrowser(info
															.getApkUrl());
												}
											})
									.setNegativeButton(R.string.laterNotify,
											null).show();
						}
					} else {
						if (curVer == ver) {
							...
							if (firstRunThisVersion) {
								...
								String title = UpdateService.this.activity
										.getString(R.string.updateLog);
								Utils.showInfoDialog(
										UpdateService.this.activity, msg, title);
							}
						}
					}
				}
	...
	public void showSureUpdateDialog() {
		new UpdateTask(activity, true, AVQuery.CachePolicy.NETWORK_ELSE_CACHE) {
			...
			public void done(final UpdateInfo info, Exception e) {
				if (e == null) {
					if (info.getVersion() > getVersionCode(activity)) {
						AlertDialog.Builder builder = Utils
								.getBaseDialogBuilder((Activity) activity);
						...
						Utils.toast(activity, R.string.versionIsAlreadyNew);
					}
				} else {
					e.printStackTrace();
					Utils.toast(activity, R.string.failedToGetData);
				}
			}
		}.execute();
	}
	...
	public abstract static class UpdateTask extends NetworkAsyncTask {
		UpdateInfo info;
		AVQuery.CachePolicy policy;

		protected UpdateTask(Context ctx, boolean openDialog,
				AVQuery.CachePolicy policy) {
