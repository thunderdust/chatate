...
public class CheckListAdapter<T> extends BaseListAdapter<T> {
	List<Boolean> checkStates = new ArrayList<Boolean>();
	boolean defaultState = false;

	public CheckListAdapter(Context ctx, List<T> datas) {
		super(ctx, datas);
	...
	void assertSize(int position) {
		if (position >= checkStates.size()) {
			// throw new
			// IllegalArgumentException("position is bigger than size");
