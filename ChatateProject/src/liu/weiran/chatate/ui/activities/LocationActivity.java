...
public class LocationActivity extends BaseActivity implements
	...
	private void gotoChatPage() {
		if (lastLocation != null) {
			...
			Utils.toast(MyApplication.mCtx, R.string.getGeoInfoFailed);
		}
	}
			...
			if (lastLocation != null) {
				if (lastLocation.getLatitude() == location.getLatitude()
						&& lastLocation.getLongitude() == location
								.getLongitude()) {
					Logger.d(MyApplication.mCtx.getString(R.string.geoIsSame));// 若两次请求获取到的地理位置坐标是相同的，则不再定位
					locClient.stop();
					return;
				}
			}
	...
	public class BaiduReceiver extends BroadcastReceiver {
		public void onReceive(Context context, Intent intent) {
			...
			if (s.equals(SDKInitializer.SDK_BROADTCAST_ACTION_STRING_PERMISSION_CHECK_ERROR)) {
				Utils.toast(mCtx,
						MyApplication.mCtx.getString(R.string.mapKeyErrorTips));
			} else if (s
					.equals(SDKInitializer.SDK_BROADCAST_ACTION_STRING_NETWORK_ERROR)) {
				Utils.toast(mCtx,
						MyApplication.mCtx.getString(R.string.badNetwork));
			}
		}
	}
	...
	public void onGetReverseGeoCodeResult(ReverseGeoCodeResult result) {
		...
		if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
			Utils.toast(mCtx,
					MyApplication.mCtx.getString(R.string.cannotFindResult));
			return;
		}
		Logger.d(MyApplication.mCtx.getString(R.string.reverseGeoCodeResultIs)
				+ result.getAddress());
	...
	protected void onDestroy() {
		...
		mapView.onDestroy();
		mapView = null;
		// cancel listening to SDK broadcast
