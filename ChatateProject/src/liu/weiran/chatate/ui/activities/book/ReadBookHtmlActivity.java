...
public class ReadBookHtmlActivity extends FragmentActivity {
	...
	private void fireNewAnnotationFragment(String highlightedText,
		...
		toNewAnnotation.putExtra("author", mAuthor);
	...
	private void initializeWebView(final WebView mWebView, int filePathIndex) {
		...
		String assetPath = "file:///android_asset/dummy.html";
	...
	private void loadAnnotatingJSFiles(WebView w) {
		...
		String injectJSURL = "javascript:(function(){console.log(\"haha\");"
	...
	private void setupBasicValues() {
		...
		mAuthor = extra.getString("author");
		Log.d(TAG, "book author: " + mAuthor);
		...
		mHtmlFilePathList = getBookHtmlPath(mBookIndex, mBookName, mAuthor);
		mHtmlFileNameList = getBookHtmlName(mBookIndex, mBookName, mAuthor);
		mHtmlFileNumber = mHtmlFilePathList.size();

	}

	private ArrayList<String> getBookHtmlName(final String bookIndex,
			final String bookName, final String author) {

		if (Environment.getExternalStorageState().equals(
				Environment.MEDIA_MOUNTED)) {
			String rootPath = Environment.getExternalStorageDirectory()
					.getPath();
			String bookFolderPath = rootPath + "/chatate/Books/" + bookIndex
					+ "-" + bookName + "-" + author;
