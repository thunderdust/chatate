...
import liu.weiran.chatate.ui.activities.NotifySettingActivity;
...
public class MySpaceFragment extends BaseFragment implements
	...
	private void refresh() {
		...
		genderView.setText(User.getGenderDescription(curUser));
	...
	private void showSexChooseDialog() {
		...
		new AlertDialog.Builder(ctx)
				...
				.setSingleChoiceItems(User.genderDescriptions, checkItem,
	...
	private String saveCropAvatar(Intent data) {
		...
		if (extras != null) {
			...
			if (bitmap != null) {
				bitmap = ImageUtils.toRoundCorner(bitmap, 10);
				String filename = new SimpleDateFormat("yyMMddHHmmss")
						.format(new Date());
				path = PathUtils.getAvatarDir() + filename;
				Logger.d("save bitmap to " + path);
				ImageUtils.saveBitmap(PathUtils.getAvatarDir(), filename,
						bitmap, true);
