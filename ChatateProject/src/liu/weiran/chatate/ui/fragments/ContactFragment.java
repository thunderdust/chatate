...
import liu.weiran.chatate.adapters.UserFriendAdapter;
import liu.weiran.chatate.base.MyApplication;
import liu.weiran.chatate.entity.SortUser;
...
import liu.weiran.chatate.ui.activities.AddFriendActivity;
import liu.weiran.chatate.ui.activities.ChatActivity;
import liu.weiran.chatate.ui.activities.GroupListActivity;
import liu.weiran.chatate.ui.activities.NewFriendRequestActivity;
import liu.weiran.chatate.ui.views.ClearEditText;
import liu.weiran.chatate.ui.views.EnLetterView;
...
public class ContactFragment extends BaseFragment implements
	...
	private void intHeader() {
		...
		headerLayout.showTitle(MyApplication.mCtx.getString(R.string.contact));
	...
	public void onClick(View v) {
		...
		if (viewId == R.id.layout_new) {
			Utils.goActivity(ctx, NewFriendRequestActivity.class);
	...
	public void onRefresh() {
		new NetworkAsyncTask(ctx, false) {
			...
			protected void doInBack() throws Exception {
				...
				friendList = UserService.getFriends();
				CacheService.registerBatchUser(friendList);
				CacheService.setfriendIDList(UserService
						.transformIds(friendList));
			}
			...
			protected void onPost(Exception e) {
				...
				if (e != null) {
					...
					setAddRequestTipsAndListView(haveAddRequest, friendList);
				}
			}
		}.execute();
	}
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position,
		...
		ChatActivity.goUserChat(getActivity(), user.getInnerUser()
				.getObjectId());
	}
	...
	private void deleteFriend(final SortUser user) {
		new SimpleNetTask(ctx) {
			@Override
			protected void doInBack() throws Exception {
				CloudService.removeFriendForBoth(AVUser.getCurrentUser(),
						user.getInnerUser());
			}
			...
			public void onSucceed() {
				Utils.toast(MyApplication.mCtx
						.getString(R.string.deleteSucceed));
				onRefresh();
			}
		}.execute();
	}
